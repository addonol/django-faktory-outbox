...
Validates atomic job registration via Django ORM and raw SQL modes,
ensuring data integrity and correct JSON serialization of payloads.
"""
...
class TestOutboxService:
    """Business logic tests for the OutboxService and FaktoryOutbox."""

    @pytest.mark.django_db(transaction=True)
    def test_should_raise_exception_outside_transaction(self) -> None:
        """Ensures push_atomic blocks execution outside transactions."""
        with pytest.raises(OutboxTransactionError) as exception_info:
            OutboxService.push_atomic(
                task_name="unsecured_task",
                custom_payload={"status": "forbidden"},
            )
    ...
    def test_should_persist_custom_dict_payload(self) -> None:
        """Verifies manual dictionary storage in custom mode."""
    ...
    def test_should_serialize_queryset_to_json(self) -> None:
        """Ensures Django QuerySets serialize into orm mode."""
        User.objects.create(username="test_ref_user")
        queryset = User.objects.all()

        with transaction.atomic():
            record = OutboxService.push_atomic(
                task_name="sync_users", queryset=queryset
            )
    ...
    def test_should_store_raw_sql_queries(self) -> None:
        """Verifies that raw SQL and parameters are persisted."""
    ...
    def test_model_should_return_human_readable_status_processed(
        self,
    ) -> None:
        """Ensures __str__ labels processed records correctly."""
        job = FaktoryOutbox(task_name="batch_job", processed=True)
        assert str(job) == "batch_job - Processed"

    def test_model_should_return_human_readable_status_failed(
        self,
    ) -> None:
        """Ensures __str__ labels quarantine failures correctly."""
        job = FaktoryOutbox(
            task_name="batch_job", processed=False, is_failed=True
        )
        assert str(job) == "batch_job - Failed (DLQ)"

    def test_model_should_return_human_readable_status_pending(
        self,
    ) -> None:
        """Ensures __str__ labels standard pending queues correctly."""
        job = FaktoryOutbox(
            task_name="batch_job",
            processed=False,
            is_failed=False,
